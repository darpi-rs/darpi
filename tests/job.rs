...
use darpi::job::{CpuJob, IOBlockingJob, Job};
use darpi::{
...
pub struct first_async_job;
#[allow(non_camel_case_types, missing_docs)]
impl first_async_job {
    async fn first_async_job<J>() -> J where J: Job {
        async { println!("first job in the background.") }.into()
    }
}
impl darpi::job::IsRequest for first_async_job {}
#[darpi::async_trait]
impl<C> darpi::RequestJobFactory<C> for first_async_job
where
    C: 'static + Sync + Send,
{
    type HandlerArgs = ();
    type Return = impl Job;
    async fn call(
        r: &darpi::Request<darpi::Body>,
        module: std::sync::Arc<C>,
        ha: Self::HandlerArgs,
    ) -> Self::Return {
        Self::first_async_job().await
    }
}
